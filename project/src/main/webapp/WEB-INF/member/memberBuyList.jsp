...
<script type="text/javascript">
	...
	var inperiod2 = {
		...
		//전체
		input : function(order_date1, order_date2, chk, minus) {
			...
			if (chk == null) {
				...
				var minusDate = new Date();
				minusDate.setDate(minusDate.getDate() + minus);
				var order_date1 = $.datepicker
						.formatDate("yy-mm-dd", minusDate);
				$('#order_date1').val(order_date1);
				$('#order_date2').val(
						$.datepicker.formatDate($.datepicker.ATOM, new Date()));
			}
...
<script type="text/javascript">
	...
	function process() {
		...
		$.ajax({
			type : 'GET',
			dataType : 'text',
			url : 'member_buyList.do?member_signDate1='
					+ $('#member_signDate1').val() + '&member_signDate2='
					+ $('#member_signDate2').val() + '&order_date1='
					+ $('#order_date1').val() + '&order_date2='
					+ $('#order_date2').val() + '&member_purchaseAmount1='
					+ $('#member_purchaseAmount1').val()
	...
	function pageFunction(pageNumber){
		...
		if($('#check').val() != 1){	
		location.href = "member_buyList1.do?member_signDate1="
			+ $('#member_signDate1').val()
			+ '&member_signDate2='
			+ $('#member_signDate2').val()
			+ '&order_date1=' + $('#order_date1').val()
			+ '&order_date2=' + $('#order_date2').val()
...
<body>
		...
		<form name="sFrm" id="sFrm" method="get"
			...
			<table cellspacing="0" cellpadding="0" border="0" width="100%"
				...
				<tr>
					...
					<td>
						<div>
							<input type="text" class="cal_search" id="order_date1"
								name="order_date1" value="" maxlength="10" style="width: 70px" />
							~ <input type="text" class="cal_search" id="order_date2"
								name="order_date2" value="" maxlength="10" style="width: 70px" />
							직접 입력시에는 “2007-01-01” 형식으로 입력해주세요.
						</div>
						<div class="mgt5">
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12')">전체</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd')">오늘</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd', -1)">어제</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd', -3)">3일간</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd', -7)">7일간</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd', -10)">10일간</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd', -20)">20일간</button></span>
							<span class="button small"><button type="button"
									onClick="inperiod2.input('order_date11', 'order_date12', 'd', -30)">30일간</button></span>
						</div>
					</td>
				</tr>
				<tr>
	...
	<!-- memberDAO1을 걸쳤는지 확인 하기 위한 값 -->
		...
		<input type="hidden" class="member_signDate2" value="${page.member_signDate2}" />
	 	<input type="hidden" class="order_date1" value="${page.order_date1}" />
		<input type="hidden" class="order_date2" value="${page.order_date2}" /> 
